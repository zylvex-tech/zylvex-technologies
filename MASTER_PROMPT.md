...
infrastructure/kubernetes/            Kubernetes manifests (namespace, deployments, services, ingress, HPA, postgres, redis, monitoring, deploy/rollback scripts)
infrastructure/terraform/            STUB (empty)
...
- **CI/CD**: 8 GitHub Actions workflows (6 CI + `update-repo-meta.yml` for repo presentation + `e2e-tests.yml` for E2E on PRs) + web-app CI in pr-checks.yml + `k8s-validate.yml` (kubeval manifest validation on PRs), Codecov integration, staging SSH deploy
...
- **Kubernetes Manifests**: Complete production-ready K8s manifests at `/infrastructure/kubernetes/`. Namespace `zylvex-production`. 7 Deployments (auth, spatial-canvas, mind-mapper, social, notifications, realtime, web-app) with 2 replicas HA, resource requests/limits, readiness+liveness probes on `/health`. 7 ClusterIP Services. NGINX Ingress with TLS (cert-manager Let's Encrypt) routing all paths under `zylvex.com`. HPA for each service (min 2, max 10, 70% CPU). PostgreSQL StatefulSet (single instance, 20Gi PVC, init scripts for 5 DBs + PostGIS). Redis deployment. Prometheus + Grafana monitoring. ConfigMap for shared config, Secrets template (never real values). `deploy.sh` (ordered apply) + `rollback.sh` scripts. `k8s-validate.yml` CI workflow runs kubeval on PRs.
...
| `k8s-validate.yml` | PR to `main` on `infrastructure/kubernetes/**` | kubeval strict validation on all K8s manifests |
...
9. ~~Kubernetes manifests + Helm charts~~ **✅ DONE** — `/infrastructure/kubernetes/` complete production-ready K8s manifests (plain YAML, no Helm). Namespace `zylvex-production`. 7 Deployments (2 replicas HA, resource requests/limits, readiness+liveness probes). 7 ClusterIP Services. NGINX Ingress with TLS (cert-manager Let's Encrypt). HPA per service (min 2, max 10, 70% CPU). PostgreSQL StatefulSet (20Gi PVC, init scripts for 5 DBs + PostGIS). Redis deployment. Prometheus + Grafana. ConfigMap + Secrets template. `deploy.sh` + `rollback.sh`. `k8s-validate.yml` CI workflow (kubeval). All `/health` endpoints updated to return `{"status":"ok","service":"<name>"}`.
